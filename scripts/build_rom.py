...
def c_rom(a):
    ...
    if len(a) > max_rom_size:
        raise RuntimeError("ROM_size input exceeds MAX_ROM_SIZE")
        return ""
    rom_size = min(max(min_rom_size, len(a)), max_rom_size)
    max_addr_size = opt_bus_width(len(a), min_rom_size, max_rom_size) - 1
    config_list = ["0x{:04x}".format(a[ix]) for ix in range(len(a))]
    config_strings = ["  " + ", ".join(config_list[8*n:min(8*(n+1),len(config_list))]) for n in range(1+len(config_list)//8)]
    config = ',\n'.join(config_strings)
    print(config_list)
    ...
    outputMessage = ("// 16k x 16 ROM machine generated by python verilog_rom()",
        ...
        config,
        "};",
        "#define CONFIG_ROM_SIZE            ({})".format(rom_size),
        )
    return '\n'.join(outputMessage)
