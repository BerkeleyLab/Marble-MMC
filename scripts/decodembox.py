...
def get_MSB(mdump, offset, size):
    ll = [(x & 0xff) for x in mdump[offset:offset+size]]
    return int.from_bytes(bytes(ll))


def get_4(mdump, offset):
    return get_MSB(mdump, offset, 4)
...
def decodeMbox(argv):
    ...
    parser.add_argument('-d', '--def_file', default=None,
                        help='Filename for mailbox definition file to be loaded')
    parser.add_argument('-s', '--store_file', default=None,
                        help='Filename to store binary data read from mailbox')
    parser.add_argument('-r', '--raw', action='store_true', default=False,
                        help='Print the raw contents of the mailbox memory.')
    ...
    if not ipAddr:
        if not os.path.exists(args.input):
            print("{} doesn't look like IP address and also doesn't appear to be an existing file".format(args.input))
            return 1
        fromFile = True
